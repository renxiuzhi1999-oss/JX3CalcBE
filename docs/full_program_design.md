...
仓库新增的 `example/simulator` 目录提供了一套完整的 Python 版本示例，用于
快速验证上述架构：

- `data.py` 实现 JSON 资源加载，覆盖技能、奇穴、装备等结构；
- `simulation.py` 以事件推进的方式模拟技能冷却、GCD、暴击与增伤；
- `tasks.py` 提供异步任务队列，配合 REST 接口返回平均 DPS 与详细日志；
- `api.py` 暴露 FastAPI 服务并挂载内置的 UI 页面，便于调试循环；
- `ui/index.html` 设计了一个深色主题的控制台，展示构建流程与结果卡片；
- `README.md` 包含环境搭建、运行命令与接口说明。

该示例与 C++ 主体共享相同的数据流，可作为调试自定义循环、验证配装思路
或撰写单元测试的轻量入口。
